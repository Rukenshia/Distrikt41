/*
	...
	Description:
	Gathers Pfirsiche and reports it to DB?
*/
...
_type = "D41_Pfirsich";
_skill = life_skillPeach;
_skillNeeded = {};
_skillAdd = {life_skillPeach = life_skillPeach + 1};
_hType = "Pfirsiche"; //für TitleText
_Werkzeug = "D41_Gartenschere";
_CheckWerkzeug = (_Werkzeug in (assignedItems player));
_GatherText = {titleText[format["Pflücke %1.", _hType],"PLAIN"];}; //gather gestartet
_GatheredText = {titleText[format["Du hast %1 %2 gepflückt.",_rndm, _hType],"PLAIN"];}; //gather beendet
...
switch(true) do
{
	...
	case (_skill == 1249 && _CheckWerkzeug): 					{_Karma = 3;	_LvL = 6; _rndm = 5;									_count = 5; 	_time = 5;	_Lcap = "MAX";	call _fnc_gather; 	[ format ["<t color='#EC891D'><t size='1'>LEVEL UP!<br/>Skillstufe %1</t></t><br/> (%2/%3s): %4/%5",_LvL, _count, _time, _skill+1, _Lcap], SzoneXW, SzoneYH, 5, 0.25 ] spawn BIS_fnc_dynamicText;};
	//Level 5
	...
	case (_skill == 49):										{_Karma = 1;	_LvL = 2; _rndm = [2,3] call BIS_fnc_selectRandom;		_count = "2-3"; _time = 5;	_Lcap = 250;	call _fnc_gather; 	[ format ["<t color='#EC891D'><t size='1'>LEVEL UP!<br/>Skillstufe %1</t></t><br/> (%2/%3s): %4/%5",_LvL, _count, _time, _skill+1, _Lcap], SzoneXW, SzoneYH, 5, 0.25 ] spawn BIS_fnc_dynamicText;};
	//Level 1
