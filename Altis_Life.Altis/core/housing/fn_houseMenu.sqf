...
{
	if((nearestObject [[7623.11,11683.2,9.91821],"Land_Offices_01_V1_F"]) == _curTarget OR (nearestObject [[12407.1,14350.3,0],"Land_Research_house_V1_F"]) == _curTarget) then {
...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owned"}) then
{
	...
	if(!isNil {_curTarget getVariable "house_owned"}) then
	{
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_Btn5 ctrlSetText "Miete zahlen";
